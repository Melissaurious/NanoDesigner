...
import sys
sys.path.append('./NanobodiesProject/dyMEAN')
from configs import AG_TYPES, RAbD_PDB, IGFOLD_TEST_PDB, SKEMPI_PDB
from configs import IMGT
from pdb_utils import Protein, Peptide, AgAbComplex2
...
def post_process(item):
    ...
    if numbering == 'none':
        ...
        try:
            ...
            cplx = AgAbComplex2.from_pdb(pdb_data_path, H, L, A,
...
def parse():
    ...
    parser = ArgumentParser(description='download full pdb data')
    parser.add_argument('--summary', type=str, default="/ibex/user/rioszemm/sabdab_antibody_Fv_allBound_18_02_2024.tsv", help='Path to summary file')
    parser.add_argument('--fout', type=str, default="/ibex/user/rioszemm/antibodies_dataset_dyMEAN_march2024.json", help='Path to output json file')
    parser.add_argument('--type', type=str, choices=['skempi', 'sabdab', 'rabd', 'igfold_test'], default='sabdab',
                        help='Type of the dataset')
    parser.add_argument('--pdb_dir', type=str, default=None, help='Path to local folder of PDB files')
    parser.add_argument('--numbering', type=str, default='imgt', choices=['imgt', 'none'],
