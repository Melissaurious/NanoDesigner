...
def parse():
    ...
    parser.add_argument('--data', type=str,default="/ibex/user/rioszemm/Final_datasets_march_2024/Nb_Ab_bound_from_cdrh3_DiffAb_format.json") #required=True, help='Path to the data file')
    parser.add_argument('--out_dir', type=str, default="/ibex/user/rioszemm/Final_datasets_march_2024/Nb_Ab_antigen_clst", help='Directory to save results. Default the same as input data.')
    parser.add_argument('--valid_ratio', type=float, default=0.1,
                        help='Ratio of validation set')
    parser.add_argument('--test_ratio', type=float, default=0.1,
                        help='Ratio of test set')
    parser.add_argument('--cluster_seq', type=str, choices=['cdr', 'antigen'], default='antigen',
                        help='Sequence to use for clustering, either cdr or antigen.')
    # parser.add_argument('--cdr', type=str,
    #                     choices=[f'H{i}' for i in range(1, 4)] + [f'L{i}' for i in range(1, 4)],
    #                     nargs='+', default='H3',
    #                     help='Cluster CDR according to which chain, None for not clustering. Only used if --cluster_seq=cdr')
    parser.add_argument('--cdr', type=str,
                        choices=[f'H{i}' for i in range(1, 4)] + [f'L{i}' for i in range(1, 4)],
                        nargs='+', default=['H3'],
                        help='Cluster CDR according to which chain, None for not clustering. Only used if --cluster_seq=cdr')
    ...
    # parser.add_argument('--cluster_seq', type=str, choices=['cdr', 'antigen'], default='cdr',
    #                 help='Sequence to use for clustering, either cdr or antigen.')
    ...
    parser.add_argument('--k_fold', type=int, default=10, help='K fold dataset. -1 for not do k-fold.' + \
...
def main(args):
    ...
    print(f'Valid entries after filtering with {args.filter} and resolution below 4.0: {len(items)}')
    ...
    if args.cluster_seq is None:
        ...
        with open(fasta, 'w') as fout:
            for item in items:
                pdb = item['pdb']
                if args.cdr is None:
                    seq = item['heavy_chain_seq']
                    if item['light_chain_seq']:
                        seq +=item['light_chain_seq']
                else:
                    seq = ''
                    for cdr in args.cdr:
        ...
        cmd = f'mmseqs cluster {db} {db_clustered} {tmp_dir} --min-seq-id 0.5'  #
        res = exec_mmseq(cmd)
        ...
        exec_mmseq(cmd)
        db_clustered = os.path.join(tmp_dir, 'DB_clu')
        cmd = f'mmseqs cluster {db} {db_clustered} {tmp_dir} --min-seq-id 0.95'  # simlarity 
        res = exec_mmseq(cmd)
        num_clusters = re.findall(r'Number of clusters: (\d+)', res)
        if len(num_clusters):
            print(f'Number of clusters: {num_clusters[0]}')
