...
def parse():
    ...
    parser.add_argument('--save_dir', type=str, help='directory to save model and logs') # Not Required, i will defin the folder to be the same of the dataset
    parser.add_argument('--template_path', type=str, required=True, help='path to template file') #added, i created a template file per training set (every fold)
...
#     def __init__(self, file_path, save_dir=None, cdr=None, paratope='H3', full_antigen=False, num_entry_per_file=-1, random=False):
    ...
    template_generator = ConserveTemplateGenerator(args.template_path) #added to use my generated conserved templates
    train_set = E2EDataset2(args.train_set, template_generator=template_generator,cdr=args.cdr, paratope=args.paratope) # added template_generator=template_generator
    valid_set = E2EDataset2(args.valid_set, template_generator=template_generator, cdr=args.cdr, paratope=args.paratope) # requires modify E2EDataset2
