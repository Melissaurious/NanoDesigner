...
from copy import deepcopy, copy  # ADD copy here
import json
import argparse
import time
import traceback
...
def openmm_relax_no_decorator(item, platform=None, properties=None, excluded_chains=None, inverse_exclude=False):
    ...
    try:
        ...
        # Enhanced platform selection with proper error handling (from v2)
        if platform is None:
            try:
        ...
        try:
            # THE CRITICAL FIX: Use platform and properties in the Simulation constructor
...
def main(args):
    ...
    try:
        ...
        if os.path.exists(args.out_file) and os.stat(args.out_file).st_size != 0:
            print("Output file exists, checking for already processed entries...")
            try:
                with open(args.out_file, 'r') as f:
                    processed_data = f.read().strip().split('\n')
                ...
                for entry in processed_data:
                    try:
                        ...
                        print(f"Warning: Invalid JSON in output file, skipping entry")
                
                print(f"Found {len(processed_mod_pdbs)} already processed entries")
                
                # Determine which entries haven't been refined
                missing_entries = []
                for item in data:
                    if item["mod_pdb"] not in processed_mod_pdbs and item.get('refined', 'No') != 'Yes':
                        missing_entries.append(item)
                
                print(f"Found {len(missing_entries)} entries that need processing")
                inputs = missing_entries
            except Exception as e:
                print(f"Error reading output file: {e}")
                print("Will process all entries from scratch")
                inputs = data
        else:
            print("Output file doesn't exist or is empty, will process all entries")
            try:
        ...
        for i, item in enumerate(inputs):
            try:
                print(f"\nProcessing item {i+1}/{len(inputs)}: {item.get('mod_pdb', 'unknown')}")
                ...
                try:
                    with open(args.out_file, 'a') as f:
                        ...
                        os.fsync(f.fileno())
                    print(f"Saved result for item {i+1}/{len(inputs)}")
                except Exception as e:
                ...
                print(f"Error processing item {i+1}/{len(inputs)}: {e}")
                import traceback
                print(traceback.format_exc())
