...
from data.pdb_utils import AAComplex, Protein, VOCAB, AAComplex2
...
class EquiAACDataset(torch.utils.data.Dataset):
    def __init__(self, file_path, save_dir=None, num_entry_per_file=-1, random=False, ctx_cutoff=8.0, interface_cutoff=12.0):
        ...
        self.mode = '1*1'  # * relaxed filter, may be present or not
    ...
    #         pdb_id, peptides = item['entry_id'], protein.peptides
    #         # self.data.append(AAComplex(pdb_id, peptides, item['heavy_chain'], item['light_chain'], item['antigen_chains']))
    #         self.data.append(AAComplex2(pdb_id, peptides, item['heavy_chain'], item['antigen_chains'],item['light_chain']))
    ...
    def preprocess(self, file_path, save_dir, num_entry_per_file):
        ...
        for line_idx, line in enumerate(tqdm(lines, desc="Processing entries")):
            try:
                item = json.loads(line)
                
                # Check if antigen_type is valid (protein OR peptide)
                if 'antigen_type' in item:
                    antigen_types = item['antigen_type']
                    if isinstance(antigen_types, str):
                        antigen_types = [antigen_types]
                    
                    # Skip if no valid antigen types
                    valid_types = {'protein', 'peptide'}
                    if not any(atype in valid_types for atype in antigen_types):
                        print_log(f'Entry {line_idx}: Invalid antigen type {antigen_types}, skipping', level='WARN')
                        skipped_entries += 1
                        continue
                
                # Check if antigen_chains exist and are not empty
                if 'antigen_chains' not in item or not item['antigen_chains']:
                    print_log(f'Entry {line_idx}: No antigen chains found, skipping', level='WARN')
                    skipped_entries += 1
                    continue
                ...
                try:
                    ...
                    print_log(f'Entry {line_idx}: PDB parsing failed for {item.get("entry_id", "unknown")}: {e}, skipping', level='ERROR')
                    skipped_entries += 1
                    continue
                except Exception as e:
                    print_log(f'Entry {line_idx}: Unexpected error parsing PDB for {item.get("entry_id", "unknown")}: {e}, skipping', level='ERROR')
                    skipped_entries += 1
                    continue

                # Extract required fields with fallbacks
                pdb_id = item.get('entry_id', item.get('pdb', f'unknown_{line_idx}'))
                peptides = protein.peptides
                heavy_chain = item.get('heavy_chain')
                antigen_chains = item.get('antigen_chains', [])
                light_chain = item.get('light_chain')  # Can be None for nanobodies
                
                # Additional validation: check if we have at least heavy chain
                if not heavy_chain:
                    print_log(f'Entry {line_idx}: No heavy chain found for {pdb_id}, skipping', level='WARN')
                    skipped_entries += 1
                    continue
                
                # Try to create AAComplex2 object
                try:
                    complex_obj = AAComplex2(pdb_id, peptides, heavy_chain, antigen_chains, light_chain)
                    self.data.append(complex_obj)
                    success_entries += 1
                except AssertionError as e:
                    print_log(f'Entry {line_idx}: AAComplex2 creation failed for {pdb_id}: {e}, skipping', level='ERROR')
                    skipped_entries += 1
                    continue
                except Exception as e:
                    print_log(f'Entry {line_idx}: Unexpected error creating AAComplex2 for {pdb_id}: {e}, skipping', level='ERROR')
                    skipped_entries += 1
                    continue
                
                # Save data in chunks if specified
                if num_entry_per_file > 0 and len(self.data) >= num_entry_per_file:
                    self._save_part(save_dir, num_entry_per_file)
                    
            except json.JSONDecodeError as e:
                print_log(f'Entry {line_idx}: JSON parsing failed: {e}, skipping', level='ERROR')
                skipped_entries += 1
                continue
            except Exception as e:
                print_log(f'Entry {line_idx}: Unexpected error: {e}, skipping', level='ERROR')
                skipped_entries += 1
                continue
        ...
        # Print summary
        print_log(f'Preprocessing complete: {success_entries} successful, {skipped_entries} skipped out of {total_entries} total entries', level='INFO')
    ...
    def __getitem__(self, idx):
        ...
        # chain_lists, begins = [], []
        # if self.mode[0] == '1':  # Include heavy chain
        #     chain_lists.append([hc])
        #     begins.append(VOCAB.BOH)  # '+'

        # # Check for light chain inclusion based on self.mode[1]
        # if self.mode[1] == '1' and lc is not None:
        #     # Include light chain only if it exists
        #     chain_lists.append([lc])
        #     begins.append(VOCAB.BOL)  # '-'
        # elif self.mode[1] == '*' and lc is not None:
        #     # Include light chain if it exists, but it's optional
        #     chain_lists.append([lc])
        #     begins.append(VOCAB.BOL)  # '-'
        # # If self.mode[1] == '0', do not include the light chain at all

        # if self.mode[2] == '1':  # Include antigen chains
        #     chain_lists.append(antigen_chains)
        #     begins.append(VOCAB.BOA)  # '&'
        ...
        # prepare input
        chain_lists, begins = [], []
        if hc:  # Include heavy chain if it exists
            chain_lists.append([hc])
            begins.append(VOCAB.BOH)
        if lc:  # Include light chain conditionally
            chain_lists.append([lc])
            begins.append(VOCAB.BOL)
        if antigen_chains:  # Include antigen chains mandatorily
            chain_lists.append(antigen_chains)
            begins.append(VOCAB.BOA)
