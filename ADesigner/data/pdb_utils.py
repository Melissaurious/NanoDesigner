...
class AAComplex2(Protein):  # Antibody-Antigen complex
    ...
    def __init__(self, pdb_id: str, peptides: Dict[str, Peptide], heavy_chain: str,
                antigen_chains: List[str], light_chain: Optional[str] = None, numbering: str='imgt',
        ...
        selected_peptides = {}
        if light_chain and light_chain in peptides:
            selected_peptides[light_chain] = peptides[light_chain]
        ...
        #     chains = [heavy_chain]
        #     if light_chain:
        #         chains.extend(light_chain)
        #     for chain_name in chains:
        #         if chain_name in peptides:
        #             selected_peptides[chain_name] = peptides[chain_name]
            
        if cdr_pos is None:
            additional_peptides, self.cdr_pos = self._extract_antibody_info(peptides, numbering)
            selected_peptides.update(additional_peptides)
        else:
            self.cdr_pos = deepcopy(cdr_pos)
            chains = [self.heavy_chain, self.light_chain] if self.light_chain else [self.heavy_chain]
            for chain_name in chains:
                if chain_name in peptides:
                    selected_peptides[chain_name] = peptides[chain_name]
    ...
    def _extract_antibody_info(self, peptides, numbering):
        ...
        chain_names = [self.heavy_chain] if self.light_chain is None else [self.heavy_chain, self.light_chain]  # Only add light_chain if it's not None
    ...
    def _cal_interface(self):
        ...
        antibody_chains = [self.get_heavy_chain()]        
        # Add the light chain to the list if it's not None
        if self.get_light_chain() is not None:
            antibody_chains.append(self.get_light_chain())
    ...
    def __str__(self):
        ...
        antibody_info = f'Antibody H-{self.heavy_chain} ({len(self.get_heavy_chain())})'
        # Conditionally handle the light chain
        if self.light_chain:
        ...
        for name in self.cdr_pos:
            ...
            if chain is None:  # If the chain is None, continue to the next iteration
                continue
