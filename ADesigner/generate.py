...
sys.path.append(os.path.join(parent_dir,"ADesigner"))
...
# def eval_one(cplx, seq, x, true_x, aligned, k, out_dir, cdr='H3'):
# def eval_one(cplx, seq, x, true_x, aligned, k, out_dir, cdr='H3', pdb_dict=None):
def eval_one(tup, out_dir, cdr='H3', pdb_dict=None):
    ...
    # For now  i will choose the mutant with the lowest PPL value, then will evaluate it properly
...
def rabd_test(args, model, test_set, test_loader, out_dir, device, pdb_dict):
    ...
    print_log(f'dumped to {args.out_dir}')
    ...
    for k in range(args.rabd_topk):
        ...
        for cplx, item in zip(test_set.data, global_best_results):
            # inputs.append((cplx, ) + item[k] + (k))  # Adding the k_index to the tuple
            inputs.append((cplx, ) + tuple(item[k]) + (k,))
        
        # cdr_type = 'H' + model.cdr_type
        summaries = process_map(partial(eval_one, out_dir=args.out_dir, cdr='H3', pdb_dict=pdb_dict), inputs, max_workers=args.num_workers, chunksize=10)


        # summary_dir = os.path.dirname(args.out_dir)
        # summary_fout = open(os.path.join(args.out_dir, f'summary_fold_{args.FOLD}.json'), 'w')
        summary_fout = open(os.path.join(args.out_dir, 'summary.json'), 'w')
        for i, summary in enumerate(summaries):
            summary['PPL'] = global_best_ppl[i][k]
            summary_fout.write(json.dumps(summary) + '\n')
        summary_fout.close()
...
def main(args):
    ...
    if os.path.exists(summary_file) and os.stat(summary_file).st_size != 0:
        print(f"Summary file for fold {args.FOLD} already exists and not empty, Skipping inference step...")
        exit()
    ...
    with open(args.test_set, 'r') as fin:
        ...
        # summary = [json.loads(item) for item in fin]
    ...
    model.to(device)
    model.eval()
    # rabd_test(args, model, test_set, test_loader, device, pdb_dict)
    rabd_test(args, model, test_set, test_loader, args.out_dir, device, pdb_dict)
    ...
    for cplx in tqdm(test_set.data):
        pdb_path = os.path.join(args.out_dir, cplx.get_id() + '_ref.pdb')
...
def parse():
    ...
    # check /ibex/user/rioszemm/NanobodiesProject/ADesigner/parser.py file
