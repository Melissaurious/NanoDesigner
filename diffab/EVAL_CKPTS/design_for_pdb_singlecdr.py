...
def create_data_variants(structure_factory):
    ...
    # print(f"CDRs found: {cdrs}")  # Add this line for debugging
        ...
        for batch in tqdm(loader, desc=variant['name'], dynamic_ncols=True):
            ...
            for i in range(aa_new.size(0)):
                ...
                try: 
                    ...
                    import sys 
                    sys.path.append(args.dymean_code)
...
def main(): 
    ...
    parser.add_argument('--dymean_code', type=str, default="", help="path to dymean code directory")
    ...
    checkpoint = os.listdir(args.ckpt_folder)[-1]
    checkpoint_full_path = os.path.join(args.ckpt_folder,checkpoint)

    # print("checpoint file = ",checkpoint_full_path)
    ...
    for json_obj in data:
        ...
        # if os.path.exists(save_path):
        # mod_pdb_path = os.path.join(args.out_dir, pdb_unique_code + ".pdb")
        # if os.path.exists(mod_pdb_path):
        #     continue
        ...
        try:
            ...
            print(error_message)
            continue
        # summary = design_for_pdb(args)
        # # summary_items.append(summary)
        # summary_items.extend(summary)
